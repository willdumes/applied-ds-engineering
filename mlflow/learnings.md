...
**KV cache is what makes token generation fast.** Every layer of the transformer has an attention step with three components: Query ("what am I looking for?", derived from the current token), Key ("what do I contain?", derived from every previous token), and Value ("here's my actual content", derived from every previous token). The model computes attention scores by comparing each Query against all Keys, then produces a weighted blend of all Values. The expensive part is computing K and V for each token (pushing it through ~64 neural network layers of matrix multiplications). The attention step itself (dot products and weighted sums) is cheap by comparison.

**KV vectors are deterministic for a given token and position.** Since K and V only depend on the token, its position, and the model weights (not on future tokens), the results are identical whether computed fresh or loaded from cache. This means caching is not an approximation. It's mathematically equivalent to a full recompute, just without the redundant matrix multiplications. To generate token 100, instead of reprocessing tokens 1-99 through every layer (64 * 100 = 6,400 operations), the model loads cached K,V for 1-99 and only processes token 100 (64 * 1 = 64 operations).

**Prompt caching applies the same principle across API calls.** Providers (Anthropic, OpenAI) persist the KV cache on their servers between requests. If the message prefix is identical (same system prompt, same earlier turns), the saved KV cache is loaded directly and only the new tokens get processed. This is analogous to a **materialized view vs. a raw table scan** in a database: the KV cache is the pre-computed result, and loading it is cheaper than re-scanning the raw data. The constraint is **exact prefix matches**. Change one token in an early message and the entire cache is invalidated, just like modifying a base table invalidates the materialized view. Anthropic charges cached input tokens at ~10% of fresh input tokens.

### Quantization

**Quantization is how a 35B parameter model fits on a laptop.** Each model weight is a decimal number (e.g., `-0.0847`). At full precision (FP16, 2 bytes per weight), 35B weights = 70GB. Quantization rounds each weight to fewer bits of precision, like rounding `3.14159` to `3.14`. The math: FP16 (2 bytes) = 70GB, Q8 (1 byte) = 35GB, Q5 (~0.66 bytes) = 23GB, Q4 (0.5 bytes) = 17.5GB. Ollama distributes Qwen 3.5 at ~Q5, which is why the model file is 23GB on disk. In memory it uses ~29GB (23GB weights + KV cache + runtime overhead for activations). Neural networks are tolerant of imprecise weights because the overall pattern across billions of parameters matters more than any single weight being exact.
...
**Dense vs Mixture of Experts (MoE).** A dense model activates every parameter on every forward pass. MoE models have a **router network** that selects a small subset of "expert" sub-networks per token, keeping most parameters dormant. Qwen 3.5 ships both: the 27B is dense (all 27B active per token), the 35B-A3B is MoE (35B total but only ~3B active, routed through 8 of 256 experts). The dense 27B is smarter for complex reasoning because 27B active params > 3B active params, even though the MoE has more total parameters. The MoE trades reasoning depth for speed and efficiency (up to 5x throughput), while retaining the broad *knowledge* stored across all 35B weights. Think of a hospital: 256 specialists on staff, but each patient sees only ~8. The dense model makes every specialist examine every patient.
